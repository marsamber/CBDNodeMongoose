...
router
  ...
  .delete(cors.corsWithOptions, authenticate.verifyUser, (req, res) => {
    res.status(405);
    res.end(
      'DELETE operation not supported on /toCook/recipes/' + req.params.recipeId
    );
  })
  .put(cors.corsWithOptions, authenticate.verifyUser, (req, res) => {
    res.status(405);
    res.end(
      'PUT operation not supported on /toCook/recipes/' + req.params.recipeId
    );
  })
  .post(cors.corsWithOptions, authenticate.verifyUser, async (req, res) => {
    try {
      ...
      res.status(400).send({ error: 'Recipe already in to cook!' });
    }
  });
...
router
  ...
  .get(cors.corsWithOptions, authenticate.verifyUser, async (req, res) => {
    try {
      const toCook = await ToCook.findById(req.params.toCookId).populate({
        path: 'recipe',
      });
      res.send(toCook);
    } catch {
  ...
  .delete(cors.corsWithOptions, authenticate.verifyUser, async (req, res) => {
    try {
      await ToCook.deleteOne({ _id: req.params.toCookId });
      res.status(204).end();
    } catch {
      res.status(404);
      res.send({ error: "To cook doesn't exist!" });
